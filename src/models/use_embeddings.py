...
def main(args):
    ...
    df_acc = pd.DataFrame(
        columns=["Using Meta", "Just Images", "Delta", "Conf", "McNemar p-value"]
    )
    M_table_cumm = np.zeros((2, 2))
    for i, (train_index, test_index) in enumerate(kf.split(X)):
        print(f"Fold {i + 1}:")
        _, _, y_pred_cat, y_pred_img, acc_cat, acc_img = test_classifier_fold(
        ...
        pd.DataFrame(
            zip(y_pred_img, y_pred_cat, val_labels[test_index]),
            columns=["base pred", "meta pred", "true label"],
        ).to_csv(
            f"results/use_metalabels_kfold/folds/{args.classifier}_{isPCA}-FOLD{i}.csv"
        )
        # Run a mcnemar test for the two classifiers
        M_table = mcnemar_table(
            y_target=val_labels[test_index],
            y_model1=y_pred_img,
            y_model2=y_pred_cat,
        )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Doing McNemar test for k-fold")
