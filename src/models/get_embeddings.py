...
train_data = UseMetaData("train", path, annotation_path, transform=ValTransforms())
...
for i, batch in enumerate(tqdm.tqdm(train_loader)):
    train_dict[i] = [
        model(batch[0].to(device)).cpu().detach().numpy(),
        batch[1].numpy(),
        batch[2].numpy(),
    ]
...
# pickle.dump(val_dict, open('data/val_embeddings.pkl', 'wb'))
